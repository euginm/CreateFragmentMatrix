class FragMatrixCreator:
    def __init__(self, bam_path, vcf_path, region, output_folder, output_prefix='', write_genotypes=False):
        ...
        if output_prefix:
            output_prefix += '_'
        output_matrix_path = output_folder / (output_prefix + self.region_name + '.frags')
        self.output_matrix = output_matrix_path.open('w')

        if write_genotypes:
            output_genotypes_path = output_folder / (output_prefix + self.region_name + '.genotypes')
            self.output_genotypes = output_genotypes_path.open('w')
    ...
    def write_fragment_matrix(self):
        self.output_matrix.write('>' + self.region_name + '\n')
        ...
        for alignment in self.alignments:
            ...
            alignment_map = {ref_pos: read_pos for read_pos, ref_pos in alignment.get_aligned_pairs()
            ...
            if not fragment:  # no informative alleles
                continue
            if len(fragment) == 1:
                if alignment.is_paired and alignment.mate_is_unmapped:  # one informative allele but the mate is mapped
                    continue
            ...
            self.output_matrix.write('\t'.join([alignment.query_name,
                                                polymorphism_to_index[min(fragment.keys())],
                                                fragment_string,
                                                qualities_string,
...
if __name__ == '__main__':
    ...
    fragMatrixCreator = FragMatrixCreator(args.get('bam_path'), args.get('vcf_path'), args.get('region'), args.get('output_folder'))
